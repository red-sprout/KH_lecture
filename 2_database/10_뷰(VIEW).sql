...
--뷰를 통해서 조각하게되면 실제 데이터가 담겨있는 테이블에 반영이 된다.
...
/*
    ...
    2) FORCE | NOFORCE
        ...
        > NOTFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이여야만 뷰가 생성되게 하는 옵션(기본값)
...
--서브쿼리에 기술된 테이블이 존재하지 않아도 뷰가 우선을 생기게 된다. 
...
--200번 사원 급여를 200만원으로 변경(SALARY >= 3000000)조건에 맞지 않는 변경
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
...
--200번 사원 급여를 200만원으로 변경(SALARY >= 3000000)조건에 맞지 않는 변경이 되지 않는다.
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID = 200;
