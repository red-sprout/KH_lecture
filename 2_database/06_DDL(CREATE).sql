...
/*
    ...
    종류 : NOT NULL, UNIQUE, CHECK, PRIMARY, FOREIGN KEY
*/
...
/*
    *UNIQUE 제약조건
    ...
    삽입/수정시 기존에 있는 데이터 값 중 중복값이 있을 경우 오류를 발생시킨다.
*/
...
--> CHECK 제약조건때문에 에러가 발생한다.
--> 만일 GENDER컬럼에 데이터를 넣고자한다면 CHECK제약조건에 만족하는 값을 넣어야 한다.
...
/*
    ...
    PRIMARY KEY 제약조건을 부여 -> NOT NULL + UNIQUE
...
INSERT INTO MEM_PRI2
VALUES(2, 'USER3', 'PASS3', '홍길순', '여', NULL, NULL);
...
/*
    *FOREIGN KEY(외래키) 제약조건
    다른테이블에 존재하는 값만 들어와야되는 특정 칼럼에 부여하는 제약조건
    -> 다른 테이블은 참조한다고 표현
    ...
    -> 참조할컬럼면 생략시 참조할 테이블에 PRIMARY KEY로 지정된 컬럼이 매칭된다.
*/
