...
public class Variable {
	/*
	 * 변수의 선언
	 * 값을 기록하기위한 변수를 메모리 공간에 확보, 할당한다. -> 즉, 값을 보관하기 위한 공간을 만든다고 생각하면 된다.
	 * 
	 * [표현법] 자료형 변수이름;
	 * 
...
//		int num;
//		
//		// boolean(참고 거짓)이라는 자료형으로 isTrue라는 이름의 변수를 선언
//		boolean isTrue;
//		
...
//		System.out.println(num1 + num2);
		...
		System.out.println(ch1 + ch2 + ch3); // 유니코드에 더한 결과인 숫자에 맞는 문자가 없어서 그냥 숫자로 나옴
		...
		/*
		 * 문자열
		 * String(객체) <-> 원시 타입(원래 있는 타입 int,...)
		 * 문자열 같은 경우 할당해야 할 메모리 크기가 너무 가변적이므로 원시타입이 아닌 객체로 변수를 만들어 준다.
		 */
		...
		/*
		 * 이스케이프 시퀀스
		 * 문자열 내에서 백스페이스, 텝, 백슬래시, 작은따옴표 등 코드로 사용되는 특수문자를 표형할 때 사용하는 방식
		 * 
		 * 백스페이스 \b
		 * 탭문자 \t
		 * 백슬래시 \\
