...
public class RunException {
	 ...
	 * - NullPointerExcpetion : 참조변수가 null로 초기화된 상태에서 해당 참조변수에 특정 변수나 메소드로 접근을 시도할 때 발생되는 예외
	 * - ClassCastException : 허용할 수 없는 형변환이 진행될 때 발생되는 예외
	 * - ArithmeticExcpetion : 나누기 연산시 0으로 나누어질 때 발생되는 예외
	 * 
	 * => RuntimeException 관련 예외는 충분히 예측가능한 상황이 있다.
	 */
...
//		System.out.println("나누기 결과 : " + result);
		...
		/*
		 * 예외처리 : 예외가 발생했을 경우에 대해서 처리하는 방법을 예외처리
		 * try {
		 * 		예외가 발생할 수 있는 구문;
		...
		try {
			...
			e.printStackTrace(); // 강제로 오류난 이력보고자 할 때 실행
		}
	...
	public void method03() throws IOException{
		...
		// Scanner와 같이 키보드로 값을 입력받릉 수 있는 객체(단, 문자열로 읽어들여진다.)
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		
		// 1. try~catch문 : 여기 이자리에서 곧바로 예외를 처리하겠다.
		// try{} : 예외가 발생할 가능성이 있는 코드를 작성
		// catch(발생될 예외클래스 매개변수) {} : try 구문 내에서 예외가 발생했을 경우 어떻게 처리할지를 정의
...
//		}
		...
		// 2. throws : 지금 이자리에서 예외를 처리하지 않고 현재 이 메소드를 호출하는 곳으로 떠넘긴다.
		String str = br.readLine();
	}
