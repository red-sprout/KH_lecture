...
public class RunTest {
	 ...
	 * 5) 결과받기 > SELECT문 실행(6-1) => ResultSet 객체(조회된 결과를 담아준다), DML(INSERT, UPDATE, DELETE)문 실행(6-2) => int
	 * 
	 * 6-1) ResultSet객체에 담겨있는 데이터들을 하나씩 추출해서 차근차근 옮겨담기[+ArrayList에 담기]
	 * 6-2) 트랜잭션 처리(성공했다면 commit, 실패했다면 rollback 실행)
	 * 
	 * 7) 다 사용한 JDBC용 객체를 반드시 자원 반납(close) => 생성된 역순
	 */
	
	// 1.각자의 pc(localhost)dp jdbc계정 연결 후 test테이블에 insert 해보자
	...
	public static void main(String[] args) {
		...
		// 실행할 sql문(완전한 상태로 만들어두기)(맨 뒤에 세미콜론 없어야된다!!!!!!!!)
		String sql = "INSERT INTO TEST VALUES(" + num + ", '" + name + "', SYSDATE)";
		
		try {
			// 1) JDBC Driver 등록
			Class.forName("oracle.jdbc.driver.OracleDriver"); // ojdbc6.jar 파일을 추가하지 않았는지 확인 | 오타
		...
		*/
		...
		try {
			...
			// rset.next() => rset의 다음행이 들어있는지 없는지 확인 후 결과 반환 + 한행을 내려줍니다
			
			while(rset.next()) {
				//현재 참조하는 rset으로부터 어떤 컬럼에 해당하는 값을 어떤타입으로 추춣할건지 제시해줘야한다.
