...
//		// (Parent 접근시 자동으로 형 변환 된 채로 진행)
//		
//		System.out.println("3. 부모타입 레퍼런스로 자식객체를 다루는 경우(다형성)");
//		Parent p2 = /*(Parent)*/ new Child1(); // 자동형변환 됨(업캐스팅)
//		p2.printParent();
//		((Child1)p2).printChild1(); // 강제형변환을 하면 호출 가능!!(다운 캐스팅)
		// p2 레퍼런스로 지금 당장은 Parent에서만 접근이 가능하지만
		 ...
		 * 
		 * 다형성 사용전...
		 * Child1 객체 2개 Child2 객체 2개 관리해야한다.
		 */
...
//		((Child2)arr[3]).printChild2();
		...
		/*
		 * instanceif 연산자 => 연산 결과 true/false로 반환
		 * 현재 레퍼런스가 실질적으로 어떤 클래스타입을 참조하는지 확인할 때 사용
		 */
		
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] instanceof Child1) { // 해당 참조변수가 실제 Child1을 참조하고 있니?(Child1공간 사용가능해?)
